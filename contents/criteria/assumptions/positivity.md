...
2. If there’s overlap across the full range, mark `Required`; if minor gaps, `Recommended`; if severe gaps, `Relaxes assumption` to specifically study this assumption’s influence or try limiting the study scope to recover overlapping covariate range. 
